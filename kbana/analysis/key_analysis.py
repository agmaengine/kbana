...
def _plot_key_stroke(contour, peak, kb_img, axis_handle, exclude_key_list=[]):
    mycmap = _transparent_cmap(plt.cm.jet)
    axis_handle.imshow(kb_img)
    cb = axis_handle.contourf(*contour, 15, cmap=mycmap)
    for m in peak:
        y, x = m[0]
        p = str(m[1])
...
def visualize_key_stroke_freq(records, keyboard_style=None, keyboard_offset=(0, 0), exclude_key_list=[], plot=False,
    ...
    if not plot:
        ...
        cb = _plot_key_stroke((x, y, z), pre_z, kb_img, ax)
        # plt.colorbar(cb)
        plt.show()
        return fig
    else:
        plt.imshow(kb_img)
        cb = _plot_key_stroke((x, y, z), pre_z, kb_img, plt)
        # plt.colorbar(cb)
        return 0


def visualize_finger_load(records):
    """
    ...
    for finger in finger_frequency:
        ...
        plt.text(*finger_points[finger], high)
        z = z + high * _gaussian2d((x, y), finger_points[finger], .07 * x.max(), .07 * y.max())
    plt.contourf(x, y, z, 15, cmap=mycmap)
